...
По умолчанию для отрисовки текста используется системный шрифт (если он был успешно найден и загружен). Если символы языка перевода отображаются некорректно, поместите в папку с сервером шрифт формата TrueType (.ttf), поддерживающий нужные символы, под именем `font.ttf`
